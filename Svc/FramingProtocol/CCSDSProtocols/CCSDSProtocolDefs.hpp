...
constexpr FwSizeType CCSDS_SDLTP_TC_MAX_FRAME_LENGTH =
    1017;  // Maximum TC transfer frame data field (CCSDS 232.0-B-4, Section 4.1.3)
constexpr FwSizeType TC_SPACE_DATA_LINK_HEADER_SIZE =
    5;  // TC Primary Header: 5 octets (CCSDS 232.0-B-4, Section 4.1.2)
constexpr FwSizeType TC_SPACE_DATA_LINK_TRAILER_SIZE =
    2;  // Frame Error Control Field (optional): 2 octets for error detection (CCSDS 232.0-B-4, Section 4.1.4)
constexpr FwSizeType TC_SPACE_DATA_LINK_MAX_TRANSFER_FRAME_SIZE =
    CCSDS_SDLTP_TC_MAX_FRAME_LENGTH + TC_SPACE_DATA_LINK_HEADER_SIZE +
...
constexpr FwSizeType TM_SECONDARY_HEADER_MAX_SIZE =
    64;  // TM Secondary Header (optional): Up to 64 octets (CCSDS 132.0-B-3, Section 4.1.3)
constexpr FwSizeType TM_OPERATIONAL_CONTROL_SIZE =
    4;  // Operational Control Field (optional): 4 octets (CCSDS 132.0-B-3, Section 4.1.5)
constexpr FwSizeType FRAME_ERROR_CONTROL_SIZE =
    2;  // Frame Error Control Field (optional): 2 octets for error detection (CCSDS 132.0-B-3, Section 4.1.6)
constexpr FwSizeType TM_DATA_FIELD_DFLT_SIZE =
    255;  // TM Data Field size varies by application, typically constrained by maximum transfer frame size (CCSDS
          // 132.0-B-2, Section 4.1.3.4)
