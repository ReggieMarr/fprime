...
namespace Svc {
  ...
  SpacePacketFramer ::
    SpacePacketFramer(const char* const compName) :
      SpacePacketFramerComponentBase(compName),
      Framer(compName)
  {
  ...
  // ----------------------------------------------------------------------
  void SpacePacketFramer ::setup(FramingProtocol& protocol) {
      FW_ASSERT(this->m_protocol == nullptr);
      this->m_protocol = &protocol;
      protocol.setup(*this);
  }

  void SpacePacketFramer ::handle_framing(const U8* const data, const U32 size, Fw::ComPacket::ComPacketType packet_type) {
      FW_ASSERT(this->m_protocol != nullptr);
  ...
  void SpacePacketFramer ::comIn_handler(const NATIVE_INT_TYPE portNum, Fw::ComBuffer& data, U32 context) {
      this->handle_framing(data.getBuffAddr(), data.getBuffLength(), Fw::ComPacket::FW_PACKET_UNKNOWN);
  }

  void SpacePacketFramer ::bufferIn_handler(const NATIVE_INT_TYPE portNum, Fw::Buffer& fwBuffer) {
  ...
  void SpacePacketFramer ::comStatusIn_handler(const NATIVE_INT_TYPE portNum, Fw::Success& condition) {
  ...
  void SpacePacketFramer ::send(Fw::Buffer& outgoing) {
  ...
  Fw::Buffer SpacePacketFramer ::allocate(const U32 size) {
